...
(function () {
    ...
    async function initIndex() {
        ...
        /* Projets filtrés par type seulement — base de calcul des compteurs sidebar */
        function byType() {
            return state.type ? projects.filter(p => p.type === state.type) : projects;
        }

        function getCounts(field, base) {
            const counts = {};
            base.forEach(p => {
                const v = p[field];
                if (!v) return;
        ...
        function buildSidebar() {
            const base = byType();
            const domainCounts = getCounts('domain', base);
            const yearCounts   = getCounts('year',   base);
            const skillCounts  = getCounts('skills', base);
            ...
            const domains = domainsOrder.filter(d => domainCounts[d] > 0);

            sidebar.innerHTML = `
                <div class="filters-group">
                    <h3>Domaine</h3>
                    <ul class="filters-list">
                        <li><a href="#" class="filter-link domain-btn${state.domain === '' ? ' active' : ''}" data-domain="">
                            <span>Tous</span><span class="count">${base.length}</span>
                        </a></li>
                        ${domains.map(d => `
                            <li><a href="#" class="filter-link domain-btn${state.domain === d ? ' active' : ''}" data-domain="${escapeHtml(d)}">
                                <span>${escapeHtml(d.replace('Developpement', 'Développement'))}</span>
                                <span class="count">${domainCounts[d]}</span>
                            </a></li>
                        `).join('')}
                    </ul>
                </div>
                ...
                <div class="filters-group">
                    <h3>Année</h3>
                    <ul class="filters-list">
                        <li><a href="#" class="filter-link year-btn${state.year === '' ? ' active' : ''}" data-year="">
                            <span>Toutes</span><span class="count">${base.length}</span>
                        </a></li>
                        ${years.map(y => `
                            <li><a href="#" class="filter-link year-btn${state.year === String(y) ? ' active' : ''}" data-year="${escapeHtml(y)}">
                    ...
                    <ul class="filters-list">
                        ${skills.map(s => `
                            <li><label class="filter-link">
                                <span><input type="checkbox" class="skill-check" value="${escapeHtml(s)}"${state.skills.includes(s) ? ' checked' : ''}> ${escapeHtml(s)}</span>
    ...
    async function initProjectPage() {
        ...
        const heroStyle = project.video ? '' : `style="background-image:url('${escapeHtml(project.image)}');"`;
        const heroVideo = project.video
            ...
            : '';
        container.innerHTML = `
            <section class="project-hero" ${heroStyle}>
                ${heroVideo}
